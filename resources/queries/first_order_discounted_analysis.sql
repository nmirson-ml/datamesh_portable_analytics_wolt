...
all_orders AS(
SELECT ft.customer_id, o.order_id, MAX(CASE WHEN had_promotion THEN 1 ELSE 0 END) AS promotion 
FROM first_orders ft
JOIN datamart.wolt_data_model_core.fact_order_item o ON o.customer_id = ft.customer_id
GROUP BY 1,2
)

SELECT customer_id, COUNT(order_id) As total_orders, SUM(promotion) AS total_orders_with_promos, SUM(promotion)/COUNT(order_id) AS ratio_of_orders_with_promo
FROM all_orders
GROUP BY 1